...
<div class="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow">
    <h1 class="text-2xl font-semibold mb-6">Post New Job</h1>

    <form action="{{ route('employer.job-postings.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200">{{ old('description') }}</textarea>
        ...
        @php
            // Check the 'from' query parameter in the URL
        ...
        <div class="flex justify-end gap-2">
            <a href="{{ $cancelUrl }}"
            class="px-4 py-2 rounded-xl border border-gray-300 text-gray-700 hover:bg-gray-100 cursor-pointer">
                Cancel
            </a>
            <button type="submit"
                    class="px-4 py-2 rounded-xl bg-emerald-500 text-white hover:bg-emerald-600 transition cursor-pointer">
                Post Job
            </button>
        </div>
    </form>
