...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Shortlisted Applicants</h1>
    <button
        type="button"
        ...
        @click="alert('Frontend-only: Export Shortlisted Applicants')"
    >
...
<div class="overflow-x-auto bg-white rounded-xl shadow border border-gray-200">
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
            @foreach(range(1,5) as $applicant)
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Applicant {{ $applicant }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Backend Developer</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">shortlisted{{ $applicant }}@example.com</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">+63 {{ rand(900000000, 999999999) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex rounded-full bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-700">Shortlisted</span>
                    </td>
