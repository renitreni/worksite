    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="h-40 sm:h-80 bg-slate-100">
            @if($employerProfile->cover_path)
                <img src="{{ asset('storage/' . $employerProfile->cover_path) }}"
                     class="w-full h-full object-cover" alt="Cover">
            @else
                <img src="{{ asset('images/cover.png') }}"
                     class="w-full h-full object-cover" alt="Philippines Flag">
            @endif
        </div>
        ...
        <div class="p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row gap-6 sm:items-start">
                ...
                <div class="-mt-14 sm:-mt-16">
                    <div class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden flex items-center justify-center">
                        @if($employerProfile->logo_path)
                            <img src="{{ asset('storage/' . $employerProfile->logo_path) }}"
                                 class="w-full h-full object-cover" alt="Logo">
                        @else
                            <div class="w-full h-full flex items-center justify-center bg-emerald-600 text-white font-black text-3xl sm:text-4xl">
                                {{ $letter }}
                            </div>
                        @endif
                    </div>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <p class="mt-1 font-bold text-slate-900 capitalize">{{ $employerProfile->status }}</p>
                        </div>
                    ...
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        ...
                        <div class="rounded-2xl border border-slate-200 p-4">
                            ...
                            <p class="mt-1 font-semibold text-slate-900">
                                ...
                                <span class="text-slate-500 font-normal">({{ $employerProfile->position ?: '—' }})</span>
                            </p>
                        </div>
                    </div>
