...
@section('content')
    ...
    <div class="space-y-6">
        ...
        @if (($pendingSubscription ?? null) && $pendingSubscription->plan)
            <div class="rounded-3xl border border-amber-200 bg-amber-50 overflow-hidden shadow-sm">
                <div class="px-6 sm:px-8 py-6 border-b border-amber-200 bg-gradient-to-r from-amber-50 to-white">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <div class="text-xs text-amber-700 font-semibold">Pending Payment</div>
                            ...
                            <div class="mt-1 text-sm text-slate-700">
                                Amount:
                                <span
                                    class="font-semibold">₱{{ number_format((float) $pendingSubscription->plan->price, 2) }}</span>
        ...
        @if ($currentSubscription ?? false)
            @php
                ...
                if ($currentSubscription->starts_at && $currentSubscription->ends_at) {
                    $total = $currentSubscription->starts_at
                        ->startOfDay()
                        ->diffInDays($currentSubscription->ends_at->startOfDay(), false);
                    $left = now()
                        ->startOfDay()
                        ->diffInDays($currentSubscription->ends_at->startOfDay(), false);
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                        <div>
                            ...
                            <div class="mt-1 flex flex-wrap items-center gap-2">
                                ...
                                <span
                                    class="inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-semibold {{ $statusPill($status) }}">
                                    {{ ucfirst($status) }}
                                </span>
                            </div>
                            ...
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-6 py-5">
                                    ...
                                    <p class="text-xs text-slate-500 truncate">
                                        ₱{{ number_format((float) ($sub->plan?->price ?? 0), 2) }}</p>
                                </td>

                                <td class="px-6 py-5">
                                    <span
                                        class="inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-semibold {{ $statusPill($sub->subscription_status) }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">Once you subscribe to a plan, it will appear
                                        here.</p>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="grid gap-5 md:grid-cols-3">
                @foreach ($plans as $plan)
                    @php
                        ...
                        $hasPending = (bool) ($pendingSubscription ?? null);
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <div class="mt-1 text-sm text-slate-600">
                                        <span
                                            class="text-3xl font-semibold text-slate-900">₱{{ number_format((float) $plan->price, 2) }}</span>
                            ...
                            <div class="mt-4 space-y-2">
                                ...
                                @foreach ($selects as $x)
                                    ...
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="text-slate-600">{{ $label }}</span>
                                        <span
                                            class="font-semibold text-slate-900 capitalize">{{ str_replace('_', ' ', (string) $val) }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="p-5 flex-1">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                @forelse($bools as $x)
                                    <span
                                        class="rounded-full bg-emerald-100 text-emerald-800 text-xs px-3 py-1 font-semibold">
                                        {{ $x->definition->label }}
                                    </span>
                                @empty
                        ...
                        @php
                            $pendingId = $pendingSubscription->id ?? null;
                            $pendingPlanId = $pendingSubscription->plan_id ?? null;

                            $isPendingPlan = $pendingSubscription && (int) $pendingPlanId === (int) $plan->id;
                        @endphp
