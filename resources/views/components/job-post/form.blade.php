@props(['action', 'method' => 'POST', 'job' => null, 'employers' => null])
    ...
    <div class="px-6 sm:px-8 py-6 space-y-8">
        ...
        @if (isset($employers))
            <div class="space-y-1">
                <label class="block text-sm font-medium text-slate-800">
                    Select Employer <span class="text-red-500">*</span>
                </label>

                <select name="employer_profile_id" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm">

                    <option value="">Choose employer</option>

                    @foreach ($employers as $emp)
                        <option value="{{ $emp->id }}"
                            {{ old('employer_profile_id', $job->employer_profile_id ?? '') == $emp->id ? 'selected' : '' }}>
                            {{ $emp->company_name }}
                        </option>
                    @endforeach
        ...
        <div class="space-y-8">
                ...
                <div class="relative">
                    <select name="industry_id" id="industry_id"
                        ...
                        @foreach ($industries ?? [] as $industry)
                            @php
                                $currentIndustryId = optional(
                                    collect($industries ?? [])->firstWhere('name', $job->industry ?? null),
                                )->id;
                            @endphp
                            <option value="{{ $industry->id }}"
                                {{ (string) old('industry_id', $currentIndustryId ?? '') === (string) $industry->id ? 'selected' : '' }}>
                                {{ $industry->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="space-y-2">
                ...
                @php
                    $oldSkillIds = old('skills', isset($job) ? explode(',', $job->skills ?? '') : []);
                @endphp
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        <select name="gender" id="gender"
                            ...
                            <option value="both"
                                {{ old('gender', $job->gender ?? 'both') == 'both' ? 'selected' : '' }}>Both
                            </option>
                            <option value="male"
                                {{ old('gender', $job->gender ?? 'male') == 'male' ? 'selected' : '' }}>Male</option>
                            <option value="female"
                                {{ old('gender', $job->gender ?? 'female') == 'female' ? 'selected' : '' }}>Female
                            </option>
                        </select>
                    </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" min="0" name="age_min" id="age_min"
                        value="{{ old('age_min', $job->age_min ?? '') }}" placeholder="e.g. 21"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" min="0" name="age_max" id="age_max"
                        value="{{ old('age_max', $job->age_max ?? '') }}" placeholder="e.g. 45"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
        ...
        <div class="space-y-8">
            ...
            <div class="space-y-1">
                ...
                <textarea name="job_description" id="job_description" rows="6"
                    ...
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">{{ old('job_description', $job->job_description ?? '') }}</textarea>
            ...
            <div class="space-y-1">
                ...
                <textarea name="job_qualifications" id="job_qualifications" rows="4"
                    ...
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">{{ old('job_qualifications', $job->job_qualifications ?? '') }}</textarea>
            ...
            <div class="space-y-1">
                ...
                <textarea name="additional_information" id="additional_information" rows="4"
                    ...
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">{{ old('additional_information', $job->additional_information ?? '') }}</textarea>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" step="0.01" name="placement_fee" id="placement_fee"
                        value="{{ old('placement_fee', $job->placement_fee ?? '') }}" placeholder="Enter amount"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
...
@php
    $oldIndustryId =
        old('industry_id') ??
        (isset($job) && isset($industries)
            ? optional(collect($industries)->firstWhere('name', $job->industry ?? null))->id
            : null);
    $oldSkillIds =
        old('skills') ??
        (isset($job) && $job->skills
            ? \App\Models\Skill::whereIn(
                'name',
                collect(explode(',', $job->skills))
                    ->map(fn($s) => trim($s))
                    ->filter(),
            )
                ->pluck('id')
...
<script>
    (function() {
        ...
        async function loadCities(country, selectedCity = null) {
            ...
            const url = `{{ route('shared.geo.cities') }}?country=${encodeURIComponent(country)}`;
        ...
        async function loadAreas(country, city, selectedArea = null) {
            ...
            const url =
                `{{ route('shared.geo.areas') }}?country=${encodeURIComponent(country)}&city=${encodeURIComponent(city)}`;
...
<script>
    (function() {
        ...
        const oldIndustryId = @json($oldIndustryId);
        const oldSkillIds = @json($oldSkillIds);
        ...
        function render(skills) {
            ...
            const oldSet = new Set(
                (oldSkillIds || []).map(id => String(id).trim())
            );
            skills.forEach(skill => {
                const checked = oldSet.has(String(skill.id).trim());
      ...
      `;
        ...
        async function load(industryId) {
            ...
            try {
                const url = `/shared/industries/${industryId}/skills`;
                const res = await fetch(url, {
                    headers: {
                        'Accept': 'application/json'
                    }
                ...
                if (!res.ok) {
                    const text = await res.text();
                    console.error('SKILLS ERROR:', res.status, text); // 🔥 IMPORTANT
