            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div class="text-lg font-semibold text-slate-900">
                        {{ $user->role === 'employer' ? ($user->employerProfile?->company_name ?: $name) : $name }}
                    </div>
            ...
            <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div class="mt-1 font-semibold text-slate-900">
                        {{ optional($user->created_at)->format('Y-m-d') ?? '—' }}</div>
                </div>
                ...
                <div class="mt-6 pt-6 border-t border-slate-200">
                    ...
                    <div class="mb-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ ucfirst($user->employerProfile?->subscription?->plan?->name ?? '—') }}</div>
                        </div>

                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-xs font-semibold text-slate-500">Subscription Status</div>
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ ucfirst($user->employerProfile?->subscription?->subscription_status ?? '—') }}</div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                        <div class="rounded-xl border border-slate-200 p-4">
                            ...
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ $user->employerProfile?->company_name ?? '—' }}</div>
                        </div>
                        ...
                        <div class="rounded-xl border border-slate-200 p-4">
                            ...
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ $user->employerProfile?->company_contact ?? '—' }}</div>
                        </div>

                        <div class="rounded-xl border border-slate-200 p-4 lg:col-span-3">
                            <div class="text-xs font-semibold text-slate-500">Address</div>
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ $user->employerProfile?->company_address ?? '—' }}</div>
                        </div>

                        <div class="rounded-xl border border-slate-200 p-4">
                            <div class="text-xs font-semibold text-slate-500">Representative</div>
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ $user->employerProfile?->representative_name ?? '—' }}</div>
                        </div>

                        <div class="rounded-xl border border-slate-200 p-4">
                            <div class="text-xs font-semibold text-slate-500">Position</div>
                            <div class="mt-1 font-semibold text-slate-900">{{ $user->employerProfile?->position ?? '—' }}
                            </div>
                        </div>

                        <div class="rounded-xl border border-slate-200 p-4">
                            <div class="text-xs font-semibold text-slate-500">DMW License</div>

                            @if ($user->employerProfile?->dmw_license_number)
                                <div class="mt-1 font-semibold text-emerald-700">
                                    {{ $user->employerProfile->dmw_license_number }}
                                </div>
                            @else
                                <div class="mt-1 font-semibold text-slate-400">
                                    Not provided
                                </div>
                            @endif
                        </div>
