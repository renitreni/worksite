...
@php
  ...
  $q = $q ?? request('q', '');
  $active = $active ?? request('active', ''); // ⭐ ACTIVE FILTER VALUE
@endphp
...
<div class="space-y-6">
  ...
  {{-- COPY THIS BLOCK TO OTHER INDEX PAGES (change route + placeholder only) --}}
  ...
  <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div class="mb-3 flex items-center justify-between">
      ...
      <p class="text-xs text-slate-500">Fields: name, active, sort order</p>
    </div>

    <form method="POST" action="{{ route('admin.skills.store') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
      @csrf
      ...
      <table class="min-w-full text-left text-sm">
        <thead class="bg-slate-50 text-xs font-semibold text-slate-600">
          <tr>
            ...
            <th class="px-5 py-3">Order & Status</th> {{-- ⭐ Inline meta update column --}}
