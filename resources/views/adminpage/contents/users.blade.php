...
@section('content')
    ...
    <div class="space-y-6" x-data="usersUI()" x-cloak>
                ...
                <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                    ...
                    @if (!$isEmployerTable)
                        <select name="verified"
                            ...
                            <option value="verified" {{ ($verified ?? '') === 'verified' ? 'selected' : '' }}>Verified
                            </option>
                            <option value="unverified" {{ ($verified ?? '') === 'unverified' ? 'selected' : '' }}>Not
                                verified</option>
                        </select>
                    @endif
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 p-5">
                ...
                <div class="mt-1 text-xs text-slate-500">
                    Showing
                    {{ $isEmployerTable ? 'Employers (with subscription)' : 'Candidates (no subscription column)' }}
                </div>
            </div>
                        ...
                        @forelse($users as $u)
                            @php
                                ...
                                if ($u->role === 'employer') {
                                    ...
                                    // ✅ plan relation (Plan model)
                                    $planModel = $sub?->plan;

                                    // ✅ string values for display/filter
                                    $planCode = $planModel?->code; // "standard" | "gold" | "platinum"
                                    $planName = $planModel?->name; // "Standard Plan" etc.

                                    $subStatus = $sub?->subscription_status;
                                    $subStarts = $sub?->starts_at;
                                    $subEnds = $sub?->ends_at;
                                }

                                $planLabel = $planName ?: ($planCode ? ucfirst($planCode) : '—');
                                $subStatusLabel = $subStatus ? ucfirst($subStatus) : '—';

                                $displayName =
                                    $u->role === 'employer' ? ($u->employerProfile?->company_name ?: $name) : $name;
                                ...
                                @if ($isEmployerTable)
                                    <td class="px-5 py-4">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            <span
                                                class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1
      {{ $planCode ? 'bg-slate-50 text-slate-700 ring-slate-200' : 'bg-white text-slate-400 ring-slate-200' }}">
                                                {{ $planLabel }}
                                            </span>
      ...
      {{ $subEnds ? 'bg-slate-50 text-slate-700 ring-slate-200' : 'bg-white text-slate-400 ring-slate-200' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $isEmployerTable ? 4 : 3 }}"
    ...
    <script>
        function usersUI() {
            return {
                ...
                get suspendAction() {
                    return this.selectedId ? `/admin/users/${this.selectedId}/suspend` : '#';
                },
                get unsuspendAction() {
                    return this.selectedId ? `/admin/users/${this.selectedId}/unsuspend` : '#';
                },
                get toggleAction() {
