        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                ...
                <p class="text-xl font-bold text-gray-900 mt-1">
                    <span data-analytic="kpis.hire_rate">—</span><span class="text-sm font-semibold text-gray-500">%</span>
                </p>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const subtleGrid = { color: PALETTE.grid, drawBorder: false };
        ...
        return new Chart(el, {
            ...
            options: {
                ...
                scales: {
                    x: {
                        grid: { display: false },
                        ticks: { color: PALETTE.ticks, maxRotation: 0, autoSkip: true, maxTicksLimit: 6 }
                    },
                    y: {
                        beginAtZero: true,
                        grid: subtleGrid,
    ...
    async function fetchAnalytics(range) {
        ...
        try {
            ...
            updateLine(chUsers,   data.series?.users ?? {labels:[],values:[]}, fmtNumber);
            updateLine(chJobs,    data.series?.jobs ?? {labels:[],values:[]}, fmtNumber);
            updateLine(chApps,    data.series?.applications ?? {labels:[],values:[]}, fmtNumber);
            updateLine(chRevenue, data.series?.revenue ?? {labels:[],values:[]}, fmtMoney);

            updateDonut(dnPay,  data.donuts?.payments_status ?? {labels:[],values:[]});
            updateDonut(dnJobs, data.donuts?.jobs_status ?? {labels:[],values:[]});
